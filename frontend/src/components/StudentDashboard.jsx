              ...
              <div className="card glass-panel" style={{ gridColumn: '1 / -1', marginTop: '1.5rem' }}>
                 ...
                 <div style={{ overflowX: 'auto' }}>
                    <table className="table" style={{ marginTop: '1rem' }}>
                        ...
                        <tbody>
                          {myReqs.map(r => {
                            const part = participants.find(p => p.event === r.event && (p.rollNo === user.username || p.studentId === user.id));
                            return (
                              <React.Fragment key={r._id}>
                                ...
                                {r.status === 'Approved' && part && (
                                  <tr>
                                    <td colSpan="6" style={{ background: 'rgba(255, 255, 255, 0.01)', padding: '1rem 2rem' }}>
                                      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                                        <Stepper currentStage={part.attendance_status} />
                                        {part.attendance_status === 'certificate-issued' && part.certificate_url && (
                                          <div style={{ marginTop: '0.75rem', textAlign: 'center' }}>
                                            <a href={part.certificate_url} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ padding: '0.4rem 1rem', fontSize: '0.8rem', background: 'var(--success)' }}>
