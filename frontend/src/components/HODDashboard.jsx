...
const HODDashboard = ({ requests, token }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="dashboard-header mb-4" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              {pendingEvents.length > 0 && (
                <span className="badge badge-warning" style={{ fontSize: '0.85rem', padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  🔔 {pendingEvents.length} Pending Event Request(s)
                </span>
              )}
              <Link to="/dashboard/analytics" className="btn-primary" style={{ padding: '0.6rem 1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div className="card glass-panel mb-4" style={{ marginTop: '1rem' }}>
                          ...
                          <tr key={e._id}>
                              ...
                              <td>
                                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                                      <button className="btn-primary" style={{ background: 'var(--success)', padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => handleApproveEvent(e._id)}>Approve</button>
                                      <button className="btn-outline" style={{ border: '1px solid var(--danger)', color: 'var(--danger)', padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => handleRejectEvent(e._id)}>Reject</button>
                                  </div>
                              </td>
                          </tr>
                      ))}
